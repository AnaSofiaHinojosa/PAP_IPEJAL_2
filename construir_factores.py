...
def factor_long_short(ret: pd.DataFrame, carac: pd.DataFrame, largo: str,
                      pesos: pd.DataFrame | None) -> pd.Series:
    """Rendimiento mensual del portafolio largo-corto. La característica y los
    pesos se toman de t-1 (shift) y el rendimiento de t."""
...
def main():
    ...
    for nombre, c, largo in FACTORES:
        factores[nombre] = factor_long_short(ret, car[c], largo, pesos)
    ...
    largo = pd.concat({k: v.stack(future_stack=True) for k, v in car.items()}, axis=1)
    ...
    definiciones = pd.DataFrame([
        ...
        ("rendimientos.csv", "Rendimiento simple mensual con precio ajustado (incluye dividendos)"),
