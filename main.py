...
main.py - Pipeline completo del PAP:
    1. Construye el universo point-in-time del S&P 500 (Datos.py)
    2. Sobrevivientes (en el S&P 500 en el año inicial y en el final) y de
       ellas el Top 100 por market cap más reciente (top100.py)
...
from top100 import SurvivorTop100Selector
...
class PAPFactorPipeline:
    ...
    def _info_empresa(self, t_orig: str, t_yahoo: str) -> pd.DataFrame:
        ...
        info = {
            ...
            "Rank global": uni.get("Rank_Global", ""),
            "Peso en SPY (año final)": uni.get("Peso_final", ""),
            "Ticker en año inicial": uni.get("Ticker_inicio", ""),
            "Empresa en año inicial": uni.get("Company_inicio", ""),
            "Selección (OK/MANUAL)": uni.get("Estado", ""),
...
def main():
    ...
    TOP_N = 100
    START_YEAR = None  # None = el año más antiguo en HISTORICAL_DIR (idealmente 2006)

    # 2. Construir el universo COMPLETO (todas las empresas del S&P 500 por
    #    año, no solo el top 100: la regla de sobrevivientes lo necesita)
    print("\n[Paso 1/6] Procesando archivos históricos y holdings de 2026...")
    ...
    builder.export_to_csv("universo_sp500_completo.csv")

    # 3. Sobrevivientes (en el índice en el año inicial Y en el final) ->
    #    top 100 por market cap más reciente (peso en el SPY del año final)
    print("\n[Paso 2/6] Seleccionando sobrevivientes y top 100 por market cap...")
    selector = SurvivorTop100Selector(df_pit, start_year=START_YEAR, top_n=TOP_N)
    df_top100_final = selector.calculate_rankings()
    selector.export_summary_to_csv("universo_final_top100_sobrevivientes.csv",
                                   revision_path="revision_sobrevivientes.csv")

    # 4. Obtener la lista de Tickers
    tickers_list = selector.get_tickers_list()
    print("\nUniverso Top 100 consolidado con éxito.")
    ...
    pipeline = PAPFactorPipeline(tickers_list, info_universo=df_top100_final)
    pipeline.ejecutar()
