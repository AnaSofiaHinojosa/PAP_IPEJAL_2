...
# ---------------------------------------------------------------------- #
# Acciones de fuentes EXTERNAS (cuando EDGAR no las trae)
# ---------------------------------------------------------------------- #
# El API de company facts omite los datos reportados POR CLASE de acción.
# Por eso BRK.B (desde 2016) y V (desde 2010) no tienen acciones ni EPS en
# EDGAR, aunque sí tienen utilidad neta (NetIncomeLoss). Se completan con:
#   1. Data/factset/SP500_AAAA.xlsx (exportaciones de FactSet Screening):
#      acciones = MktVal Co (millones) / Closing Price. Como FactSet ajusta
#      el precio por splits posteriores, quedan en la base de acciones de
#      HOY, igual que Yahoo. MktVal Co es de la COMPAÑÍA (todas las clases),
#      así que para BRK.B da acciones "equivalentes clase B".
#   2. Data/acciones_externas.csv (manual): columnas ticker, fecha, acciones
#      (ya en base de hoy; BRK.B en equivalentes clase B).
# Solo se usan para rellenar huecos de EDGAR (sin dato de EDGAR a menos de
# 120 días) y para calcular EPS = utilidad neta / acciones donde no hay EPS.
ARCHIVO_ACCIONES_EXTERNAS = Path("Data/acciones_externas.csv")
PATRON_FACTSET = "Data/factset/SP500_*.xlsx"
DIAS_ARRASTRE_EXTERNO = 365   # un dato externo anual vale hasta 1 año


def _leer_factset_acciones(patron: str = PATRON_FACTSET) -> pd.DataFrame:
    filas = []
    for archivo in sorted(glob.glob(patron)):
        m = re.search(r"(\d{4})", Path(archivo).name)
        if not m:
            continue
        try:
            ...
            if len(fila) == 0:
                continue
            df = pd.read_excel(archivo, header=int(fila[0]))
            df.columns = [str(c).strip() for c in df.columns]
            if not {"Symbol", "MktVal Co", "Closing Price"} <= set(df.columns):
                continue
            mc = pd.to_numeric(df["MktVal Co"], errors="coerce") * 1e6
            px = pd.to_numeric(df["Closing Price"], errors="coerce")
            filas.append(pd.DataFrame({
                "ticker": df["Symbol"].astype(str).str.strip().str.upper(),
                "fecha": pd.Timestamp(f"{m.group(1)}-12-31"),
                "acciones": mc / px,
                "fuente": f"FactSet {Path(archivo).name}",
            }))
        except Exception:
            continue
...
def cargar_acciones_externas() -> dict[str, pd.DataFrame]:
    """{ticker: DataFrame(fecha, acciones, fuente)} de FactSet + manual."""
    partes = [_leer_factset_acciones()]
    if ARCHIVO_ACCIONES_EXTERNAS.exists():
        man = pd.read_csv(ARCHIVO_ACCIONES_EXTERNAS)
        man["ticker"] = man["ticker"].astype(str).str.strip().str.upper()
        man["fecha"] = pd.to_datetime(man["fecha"])
        man["fuente"] = "manual"
        partes.append(man[["ticker", "fecha", "acciones", "fuente"]])
    todo = pd.concat([p for p in partes if not p.empty], ignore_index=True) if any(
        not p.empty for p in partes) else pd.DataFrame()
    if todo.empty:
        return {}
    todo = todo.dropna(subset=["acciones"])
    todo = todo[todo["acciones"] > 0]
    # la manual gana sobre FactSet en la misma fecha
    todo["_m"] = (todo["fuente"] == "manual").astype(int)
    todo = (todo.sort_values(["ticker", "fecha", "_m"])
            .drop_duplicates(["ticker", "fecha"], keep="last").drop(columns="_m"))
    return {t: g.sort_values("fecha").reset_index(drop=True) for t, g in todo.groupby("ticker")}
...
class SecEdgarFundamentalsFetcher:
    ...
    def __init__(self, tickers: list[str], pausa_entre_requests: float = 0.15,
        ...
        self.acciones_externas = cargar_acciones_externas()
        if self.acciones_externas:
            print(f"  Acciones externas (FactSet/manual) para {len(self.acciones_externas)} tickers")
    ...
    def _eps_calculada(self, facts: dict) -> pd.DataFrame:
        ...
        # EPS = utilidad neta / acciones EXTERNAS (BRK.B, V): la acción vigente
        # al cierre de cada trimestre (dato externo más reciente, máx. 1 año).
        ext = self.acciones_externas.get(self._ticker_actual)
        if ext is not None and not ext.empty:
            for pu, (tax_u, tag_u, un_u) in enumerate(UTILIDAD_TAGS):
                ni = self._trimestres_point_in_time(self._registros(facts, tax_u, tag_u, un_u))
                if ni.empty:
                    continue
                m = pd.merge_asof(ni.sort_values("end"),
                                  ext[["fecha", "acciones"]].rename(columns={"fecha": "_f"}).sort_values("_f"),
                                  left_on="end", right_on="_f", direction="backward",
                                  tolerance=pd.Timedelta(days=DIAS_ARRASTRE_EXTERNO))
                m = m.dropna(subset=["acciones"])
                if len(m) >= 4:
                    m["valor"] = m["valor"] / m["acciones"]
                    m["factor"] = 1.0
                    m["tag"] = f"calculado: {tag_u} / acciones externas"
                    m["origen"] = m["origen"] + " (EPS calculada, acciones externas)"
                    m = m.drop(columns=["acciones", "_f"]).reset_index(drop=True)
                    # prioridad baja: solo gana si llega más lejos que las otras
                    opciones.append((m["end"].max(), -1000 - pu, m))
                break
        if not opciones:
            return pd.DataFrame()
    ...
    def _calculada_coincide(self, rep: pd.DataFrame, calc: pd.DataFrame) -> bool:
        ...
        m = m.dropna()
        m = m[m["rep"].abs() > 0.05]
        externa = calc["tag"].astype(str).str.contains("acciones externas").any()
        if len(m) < 4 and externa:
            return True   # sin trimestres comunes para comparar: se confía en la fuente externa
        if len(m) < 4:
            ok = False
        else:
    ...
    def _externas(self) -> pd.DataFrame:
        """Acciones externas del ticker actual en el formato de _registros.
        Cada dato anual se repite cada ~90 días hasta el siguiente (máx. 1
        año), para que no se pierda por DIAS_MAX_VIGENCIA."""
        ext = self.acciones_externas.get(self._ticker_actual)
        if ext is None:
            ext = self.acciones_externas.get(normalizar_ticker_sec(self._ticker_actual).replace("-", "."))
        if ext is None or ext.empty:
            return pd.DataFrame()
        filas = []
        fechas = list(ext["fecha"]) + [None]
        for i, r in ext.iterrows():
            siguiente = fechas[i + 1]
            for d in range(0, DIAS_ARRASTRE_EXTERNO, 90):
                f = r["fecha"] + pd.Timedelta(days=d)
                if siguiente is not None and f >= siguiente:
                    break
                filas.append({"end": r["fecha"], "filed": f, "valor": float(r["acciones"]),
                              "form": "externo", "fy": None, "fp": None,
                              "tag": f"externo: {r['fuente']}", "origen": "externo",
                              "factor": 1.0})
    ...
    def _acciones(self, facts: dict) -> pd.DataFrame:
        ...
        externas = self._externas()
        if not candidatos:
            if not externas.empty:
                return externas.sort_values("filed").reset_index(drop=True)
        ...
        if not externas.empty:   # al final: solo donde EDGAR no tiene nada
            mejor = self._rellenar_huecos(mejor, [externas])
